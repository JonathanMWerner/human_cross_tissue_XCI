...
#Sample metadata: /data/GTEx/samples/sample_metadata.txt

# Define files and metadata to read in
#"/home/werner/xchrom_snp_skew/bin/GTEx_ready.txt"       original files
#100 samples small_sample_test.txt
#500 samples GTEx_GATK_mod_filt_test_samples.txt

files= read.table("/home/werner/xchrom_snp_skew/bin/GTEx_ready_with_v8.txt") # example of files, should have SRR ids
#mapping = read.table("/data/GTEx/samples/sample_metadata.txt", sep="\t")          # compiled metadata from samples, original with V7
#Metadata including the v8 data
mapping = read.table('/home/werner/xchrom_snp_skew/data/GTEx/samples/sample_metadata_with_v8.txt', sep="\t", header = TRUE)
...
#For the modified wig and vcf files
#/GATK_mod_filt/
wig_files = paste0(files[meta.filter,1],"/GATK_mod_filt/chrX.intersect.wig") # WIG files from runs , input_files = wig_files
vcf_files = paste0(files[meta.filter,1],"/GATK_mod_filt/chrX.cut")           # VCF files from runs

#For non modified wig and vcf files, the original GATK parameters
#wig_files = paste0(files[meta.filter,1],"/chrX.intersect.wig") # WIG files from runs , input_files = wig_files
#vcf_files = paste0(files[meta.filter,1],"/chrX.cut")           # VCF files from runs


#Add the rest of the file name
wig_files = paste0("/data/GTEx/output/", wig_files)
vcf_files = paste0("/data/GTEx/output/", vcf_files)
...
header = '/data/GTEx/output/'
tail = '/GATK_mod_filt/chrX.snps_filtered.tab'
...
for( i in 1:length(wig_files)){
  ...
  #Above is screening only for single nucleotide substitutions, trying below to include all kinds of variants
  #For variants other than single base substitutions, the wig matches the read number/base to the last base of the indel
  #n = nchar(as.character(temp.vcf[,5]))
  #ref1 = substring(as.character(temp.vcf[,5]),n,n) == "A"
  #ref2 = substring(as.character(temp.vcf[,5]),n,n) == "C"
  #ref3 = substring(as.character(temp.vcf[,5]),n,n) == "G"
  #ref4 = substring(as.character(temp.vcf[,5]),n,n) == "T"
  
  #n = nchar(as.character(temp.vcf[,6]))
  #alt1 = substring(as.character(temp.vcf[,6]),n,n) == "A"
  #alt2 = substring(as.character(temp.vcf[,6]),n,n) == "C"
  #alt3 = substring(as.character(temp.vcf[,6]),n,n) == "G"
  #alt4 = substring(as.character(temp.vcf[,6]),n,n) == "T"
...
#Original data
#save(mat.sd, mat.avg, list.vcf, list.x, skip, files,  list.vcf.filts, wig_files, vcf_files, mapping, meta.filter, f.m, 
#file="/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/500_sample_GATK_original.Rdata")
```
...
#file="/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/100_sample_GATK_mod.Rdata")
#file="/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/all_v7_GATK_mod.Rdata")
file="/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/all_v8_GATK_mod.Rdata")
...
```{r}
load("/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/all_v8_GATK_mod.Rdata")
#load("/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/all_v7_GATK_mod.Rdata")
#load("/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/500_sample_GATK_original.Rdata")
```
...
```{r}
attr[attr$name %in% c('XIST', 'TSIX'), ]

```


Check that XIST data is present

```{r}

attr[attr$name == 'XIST', ]
xist_start = attr[attr$name == 'XIST','start' ]
xist_end = attr[attr$name == 'XIST','end' ]
xist_ensembl = attr[attr$name == 'XIST','ensemblID' ]
#Get the samples that captured heterozygous SNPs on XIST
sample_xist_index = c()
for( i in 1:length(list.vcf)){
  ...
  snp_pos = list.vcf[[i]][[1]]
  if(sum(snp_pos > xist_start & snp_pos < xist_end) > 0){
    sample_xist_index = c(sample_xist_index, i)
    
    }
}

#Look at the total XIST expression from all those samples
xist_exp = c()
for(i in sample_xist_index){

  positions = list.vcf[[i]][[1]]
  xist_filt = positions > xist_start & positions < xist_end
  
  xist_exp = c(xist_exp, list.vcf[[i]][xist_filt,'A'] + list.vcf[[i]][xist_filt,'C'])
}

hist(log10(xist_exp), main = 'Total expression of XIST het or Alt homo SNPs from 2978 samples', breaks=32, freq = FALSE)
lines(density(log10(xist_exp), na.rm=TRUE), lwd = 3)
```
...
star_header = '/data/GTEx/STARouts/'
star_tail = '.ReadsPerGene.out.tab'

star_outs = paste0(files, star_tail)
star_outs = paste0(star_header, star_outs)

star_xist_expression = vector(mode = 'numeric', length = length(star_outs))

#Load in one star_out and save the index for whaere XIST expression is
test = read.delim(star_outs[1])
ensembles = unlist(lapply(strsplit(as.character(test[ ,1]),'.', fixed = TRUE ),'[[', 1 )) #Split up the ensemble IDs from their versions
xist_star_index = which(ensembles == xist_ensembl)

for(i in 1:length(star_outs)){
  #Load the file
  read_counts = read.delim(star_outs[i])
  star_xist_expression[i] = read_counts[xist_star_index, 2] #Grab Xists expression
}
...
hist(log10(star_xist_expression), main = 'STAR quantified XIST expression for all 3487 samples', breaks = 32, xlim = c(0,5), freq = FALSE)
lines(density(log10(star_xist_expression)), lwd = 3)
```

```{r}
#Distribution of total XIST expression in the samples that have called het of Alt homo SNPs in XIST

hist(log10(star_xist_expression[sample_xist_index]), main = 'STAR quantified XIST expression for samples with alt XIST alleles', breaks = 32, xlim = c(0,5), freq = FALSE)
lines(density(log10(star_xist_expression[sample_xist_index])), lwd = 3)


```

```{r}
length(star_xist_expression)

```

So XIST is highly expressed in nearly all samples, 
...
wait to do the highly expressed filtering, reads < 3000
...
xist_exp = c()
xist_count = 0
for(i in 1:length(list.vcf.snp.filtered)){

  positions = list.vcf.snp.filtered[[i]][[1]]
  xist_filt = positions > xist_start & positions < xist_end
  if(sum(xist_filt, na.rm=TRUE) > 0){xist_count  = xist_count + 1}
  
  xist_exp = c(xist_exp, list.vcf.snp.filtered[[i]][xist_filt,'A'] + list.vcf.snp.filtered[[i]][xist_filt,'C'])
}

hist(log10(xist_exp), main = 'Total expression of XIST het or Alt homo SNPs from 2978 samples', breaks=32)
#lines(density(log10(xist_exp), na.rm=TRUE), lwd = 3)
xist_count
```
...
```{r}
sample = sample_xist_index[5]
sample
positions = list.vcf.snp.filtered[[sample]][[1]]
xist_filt = positions > xist_start & positions < xist_end
list.vcf.snp.filtered[[sample]][xist_filt, ]
...
for( i in 1:length(list.skew.max)){
  if( sum(list.skew.max[[i]]$name == 'XIST') >0 ){xist_count = xist_count + 1}
}
xist_count
...
```{r}
save(list.skew, list.skew.max,ratios.max.genes,ratios.max, Ns, file="/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/all_v8_GATK_mod_stat_filt_1_20_21.skew.est.max.genes.Rdata")
#file="/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/all_v7_GATK_mod_stat_filt_11_5_20.skew.est.max.genes.Rdata")
...
#####################################################
Sticking with just the additional stat filtered SNP lists
In addition, save an R object with the highly expressed SNPs filtered out
Also filtering out the highly reference skewed genes
####################################################
...
Found out my implementation of the R version of MLE really messes upsometimes, just not going to use it.
...
#load("/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/all_v7_GATK_mod_stat_filt_11_5_20.skew.est.max.genes.Rdata")
```
...
Lookinng at the filtered UPIC samples

```{r}
final_filt.list.skew.max[upic_index]
...
Grab the XIST skews here per sample and save for later
```{r}

xist_ref_skews = vector(mode='numeric', length = length(final_filt.list.skew.max))
xist_exp = vector(mode='numeric', length = length(final_filt.list.skew.max))

for( i in 1:length(final_filt.list.skew.max)){
  
  if(i %in% skip){xist_ref_skews[i] = NA; next}
  
  #Grab the reference skew and total expression of XIST for each sample
  xist_filt = final_filt.list.skew.max[[i]]$name == 'XIST'
  if(sum(xist_filt) == 0){xist_ref_skews[i] = NA; next}
  xist_ref_skews[i] = final_filt.list.skew.max[[i]]$C.1[xist_filt]
  xist_exp[i] = final_filt.list.skew.max[[i]]$A[xist_filt]
}
...
xist_count = 0
for( i in 1:length(final_filt.list.skew.max)){
  if( sum(final_filt.list.skew.max[[i]]$name == 'XIST') >0 ){xist_count = xist_count + 1}
}
xist_count
```
...
xist_df = data.frame(xist_ref_skews = xist_ref_skews, xist_exp = xist_exp)
xist_ratios_p = ggplot(xist_df, aes(xist_ref_skews, log10(xist_exp))) + geom_point(alpha=1/8,size=1) + ylab('log SNP reads') + xlab('SNP ref/total ratio') + ggtitle('XIST ratios vs Reads') + 
...
ggMarginal(xist_ratios_p, type='density',fill = '#752A8C')


```

check out a different gene
...
for( i in 1:length(final_filt.list.skew.max)){
  ...
  #Grab the reference skew and total expression of XIST for each sample
  xist_filt = final_filt.list.skew.max[[i]]$name == 'MECP2'
  if(sum(xist_filt) == 0){xist_ref_skews[i] = NA; next}
  xist_ref_skews[i] = final_filt.list.skew.max[[i]]$C.1[xist_filt]
  xist_exp[i] = final_filt.list.skew.max[[i]]$A[xist_filt]
}
...
xist_df = data.frame(xist_ref_skews = xist_ref_skews, xist_exp = xist_exp)
xist_ratios_p = ggplot(xist_df, aes(xist_ref_skews, log10(xist_exp))) + geom_point(alpha=1/8,size=1) + ylab('log SNP reads') + xlab('SNP ref/total ratio') + ggtitle('MECP2 ratios vs Reads') + 
  theme(plot.title=element_text(hjust=.5, size=14), axis.title.x = element_text(size=14), axis.text.x = element_text(size=12),
        axis.title.y = element_text(size=14), axis.text.y = element_text(size=12)) + 
  geom_vline(xintercept=.5, color='red')
ggMarginal(xist_ratios_p, type='density',fill = '#752A8C')
...
ggsave(filename='all_called_snps_after_GATK_filtering_v8.pdf', device='pdf', path = '/home/werner/xchrom_snp_skew/code/graphs/summary_graphs', units='in', height=5, width=7, dpi=300, useDingbats = FALSE)

```

```{r}
num_snps_sample = vector(mode = 'numeric', length = length(final_filt.list.skew.max))

for(i in 1:length(final_filt.list.skew.max)){
  if(i %in% skip){num_snps_sample[i] = 0;next}
  num_snps_sample[i] = dim(final_filt.list.skew.max[[i]])[1]
}
x = 1:length(final_filt.list.skew.max)
num_snps_sample = num_snps_sample[order(num_snps_sample)]

plot(x, num_snps_sample)
...
#check_1 = vector(mode='numeric', length = length(final_filt.list.skew.max))
check_1 = sapply(1:length(final_filt.list.skew.max), function(i) dim(final_filt.list.skew.max[[i]])[1]) 
...
check_2 = sapply(1:length(final_filt.list.skew.max), function(i) dim(final_filt.list.skew.max[[i]])[1]) 
...
```{r}
plot(unlist(check_1), unlist(check_2))
...
Also get the sample index of each gene
...
```{r}
ordered_skew_gene_names

```



Save all the gene distributions
```{r}


for(i in 1:length(ordered_skew_gene_names)){
  
  #Get the index, following the ordering of the Skew coefficient
  index = which(genes == ordered_skew_gene_names[i])
  
  data = compiled_gene_data[[index]]$C.1
  n = length(data)
  title = sprintf('Gene: %s  Num Samps: %g',ordered_skew_gene_names[i], n )
  
  filename = sprintf('/home/werner/xchrom_snp_skew/code/graphs/ref_bias_gene_distributions/%s.pdf',ordered_skew_gene_names[i] )
  pdf(file = filename)
  hist(compiled_gene_data[[index]]$C.1, main=title, xlim = c(0,1), xlab = 'Ref / Total', breaks=50)
  dev.off()
...
```{r}
#Filter out the genes with <=100 samples
...
for(i in 1:length(test)){
  ...
  n = length(data)
  title = sprintf('Gene: %s  Num Samps: %g',temp_order_genes[i], n )
...
```{r}
#Order by dip test pvalue
index = match(names(ordered_dip_p_vals), genes)
test = compiled_gene_data
...
for(i in 1:length(test)){
  ...
  n = length(data)
  title = sprintf('Gene: %s  Num Samps: %g  p-val: %f',names(ordered_dip_p_vals)[i], n, ordered_dip_p_vals[i] )
...
Find the SNPs that have that weird almost 100% reference skews
```{r}
high_ref_skew_genes = c()
samples_present = c()
for( i in 1:length(final_filt.list.skew.max)){
  
  ref_skews = final_filt.list.skew.max[[i]]$C.1
  index = ref_skews >= .95
  high_ref_skew_genes = c(high_ref_skew_genes, as.character(final_filt.list.skew.max[[i]]$name[index]) )
  if(sum(index) > 0 ){samples_present = c(samples_present,i)}
}
high_ref_skew_genes
samples_present
...
Look at PIN4 SNP skews

```{r}
index = which(genes == 'PIN4')
data = compiled_gene_data[[index]]$C.1
n = length(data)
title = sprintf('Gene: %s  Num Samps: %g',genes[index], n )
hist(data, main=title, xlim = c(0,1), xlab = 'Ref / Total', breaks=50)
...
Save a few examples of genes with bias to investigate further

```{r}

#Get the sample index the gene is detected in, to link back up to the bam files later
gene_index_func = function(gene_name, data, index){
  present = data[data$name == gene_name, ]
  if(nrow(present) >= 1){return(index)}
  else(return(NA))
}


genes_of_interest = c('PIN4','IRAK1','KDM5C')


for(i in 1:length(genes_of_interest)){

  #Get a gene
  goi_index = which(genes == genes_of_interest[i])
  #Get the sample indices for that gene
  temp_sample_index = unlist(lapply(not_skip, function(x) gene_index_func(genes[goi_index], final_filt.list.skew.max[[x]], x)))
  temp_sample_index = temp_sample_index[!is.na(temp_sample_index)]
  #Get the SNP expression info for that gene
  temp_gene_df = data.frame(rbindlist(lapply(not_skip, function(x) gene_filt(genes[goi_index], final_filt.list.skew.max[[x]]))))
  
  #Rename colnames
  colnames(temp_gene_df)[1:6] = c('snp_position','ref_counts','alt_counts','total_counts','ref_ratio','alt_ratio')
  #Add the sample names to the dataframe
  temp_gene_df$sample_id= as.character(files[temp_sample_index])
  
  #Save dataframe as text file
  file_name = sprintf('/home/werner/xchrom_snp_skew/code/ref_biased_gene_examples/%s_snp_exp.txt', genes_of_interest[i])
  write.csv(temp_gene_df , file = file_name, row.names = F)
  #Find the SNP/s that's throwing the weird bias
  par(las = 1, pty = 's')
  pos = temp_gene_df$snp_position
  ref_ratio = temp_gene_df$ref_ratio
  plot_name = sprintf('/home/werner/xchrom_snp_skew/code/ref_biased_gene_examples/%s_snp_ref_exp_plot.pdf', genes_of_interest[i])
  pdf(file = plot_name)
  plot(ref_ratio, pos, main = genes_of_interest[i], cex.axis = .75)
  dev.off()

}
```

Figure out the exact SNPs causing problems

```{r}


#Get a gene
goi_index = which(genes == 'KDM5C')
#Get the sample indices for that gene
temp_sample_index = unlist(lapply(not_skip, function(x) gene_index_func(genes[goi_index], final_filt.list.skew.max[[x]], x)))
temp_sample_index = temp_sample_index[!is.na(temp_sample_index)]
#Get the SNP expression info for that gene
...
pos = temp_gene_df$snp_position
ref_ratio = temp_gene_df$ref_ratio
plot(ref_ratio, pos, cex.axis = .75)
```

```{r}
temp_gene_df

temp_df = temp_gene_df[temp_gene_df$snp_position == 53192267 & grepl('GTEX',temp_gene_df$sample_id), ]
dim(temp_df)
plot(temp_df$ref_ratio, temp_df$snp_position)
...
```{r}
bad_index = which(genes %in% bad_genes)
...
plot(x, num_snps_sample, main = 'Number of SNPs per sample for skew estimates')
...
ggsave(filename='all_called_snps_after_ref_bias_and_highly_expressed_filtering_v8.pdf', device='pdf', 
path = '/home/werner/xchrom_snp_skew/code/graphs/summary_graphs', units='in', height=5, width=7, dpi=300, useDingbats = FALSE)
...
Parrelized version, speeds it up ridicoulously wel
...
temp = Ns[order(Ns)]
x = 1:length(Ns)
plot(x, temp, main = 'Number of SNPs in each sample', xlab = 'Sample index', ylab = 'Num filtered SNPs')
```




Look at XIST skews to the skew of the sample
```{r}

xist_skew = vector(mode = 'numeric', length = length(list.skew.max))
xist_exp = vector(mode = 'numeric', length = length(list.skew.max))

for(i in 1:length(list.skew.max)){
  
  xist_index = as.character(list.skew.max[[i]]$name) == 'XIST'
  if(sum(xist_index)==0){xist_skew[i] = NA; xist_exp[i] = NA; next}
  xist_skew[i] = list.skew.max[[i]]$C.1[xist_index]
  xist_exp[i] = list.skew.max[[i]]$A.1[xist_index]
}


xist_present_index = which(!is.na(xist_skew))
plot(est_skew[xist_present_index], xist_skew[xist_present_index], xlim = c(.5,1), xlab = 'Folded estimated tissue skew', ylim = c(0,1), ylab = 'Unfolded XIST reference skew',
     main = 'Samples with detected het XIST SNPs')

plot(folded(xist_skew[xist_present_index]), xist_exp[xist_present_index],xlim = c(0,1), xlab = ' XIST skew', ylab = 'XIST expression' )


xist_skew = folded(xist_skew)
par(pty='s')
plot(est_skew[xist_present_index], xist_skew[xist_present_index], xlim = c(.5,1), xlab = 'Folded estimated tissue skew', ylim = c(.5,1), ylab = 'Folded XIST skews',
     main = 'Samples with detected het XIST SNPs', cex=.5)
abline(a=0, b=1, col='red')

skew_deviance = est_skew[xist_present_index] - xist_skew[xist_present_index]
plot(skew_deviance, xist_exp[xist_present_index] , xlab = 'XIST skew deviance from donor skew')

```

```{r}
table(!is.na(xist_skew))
...
```{r}
#save(list.skew.max, Ns, ratios.max, file = "/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/all_v7_GATK_final_filt_2.skew.est.max.genes.Rdata")
...
#"/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/all_v7_GATK_11_5_2020_update_filt_escape.skew.est.max.genes.Rdata")
```
...
#load("/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/all_v7_GATK_11_5_2020_update_filt_escape.skew.est.max.genes.Rdata")
...
Looking at actual SNP distributions

```{r}

test_index = which(est_skew >= .5 & est_skew<= .51)
length(test_index)
for(i in test_index){
  sample = list.skew.max[[i]]
  folded_skews = folded(sample$C.1)
  title = sprintf('Skew estimate: %f',est_skew[i] )
  hist(folded_skews, breaks=16, xlim = c(.5,1), main = title)
  abline(v =est_skew[i], col='red' )
}
```
...
Seeing which genes follow the skew of the tissue, especially looking at XIST
Can we use XIST as the skew estimate for the .5-.6 skewed tissues?
#########################################################
...
```{r}
table(corrected_correlation_pvalues>=.10 & gene_skew_num_samps >= 1000 )

```

```{r}
table(names(gene_skew_num_samps) == keep_genes)
...
gene_skew_corr_df[gene_skew_corr_df$correlation_to_tissue_skew >= .8 & gene_skew_corr_df$num_samples >= 100, ]
...
kind of well correlated MECP2
well correlated TCEAL3
poorlly correlated SHROOM4
...
```{r}
file_name = sprintf('/home/werner/xchrom_snp_skew/code/graphs/gene_skew_to_tissue_skew/%s_gene_skew_to_tissue_skew_ex.pdf', current_gene)
pdf(file = file_name , height = 4, width = 4, useDingbats = FALSE)
par(pty = 's')
...
ggplot(gene_skew_corr_df, aes(x = avg_expression, y = correlation_to_tissue_skew)) + geom_point(alpha = .5) +
  ...
  geom_point(data = annotation_data, aes(x = avg_expression, y = correlation_to_tissue_skew, col = 'red')) + 
  geom_label_repel(data = annotation_data, aes(label = gene_name), size = 4, nudge_y = .1, nudge_x = .1) +
  theme(plot.title = element_text(hjust = .5), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        panel.border = element_rect(colour = "black", fill=NA, size=1), 
        axis.text.x = element_text(size = 12), axis.text.y = element_text(size = 12), 
...
plot somme examples for gene skew to tissue skew correlation
kind of well correlated MECP2
well correlated
poorlly correlated RAP2C
...
And now using the EGAD code for the proper convolution smoothing, making some minor changes 

```{r}
#Contains the conv_smoother() function
source('/home/werner/tools/analysis_R_scripts/conv_smoother.R')
```

```{r}

#Modifying it to show two SDs around the average
conv_smoother <- function(X, Y, window, raw = FALSE, output=FALSE, ...) {
    ...
    xmin <- min(X)
    X_c = m
    X_c = X_c[(window/2):(length(m) - (window/2))] 
    #X_c <- m[(((1:i) * window) - window/2)]
    Y_c <- convolve(Y[n], rep(1, window)/window, type = "filter")
    #Y_c <- Y_c[(1:i) * window - (window - 1)]
    ...
    plot(X_c, Y_c, ylim = c(0,1), xlim = c(xmin, xmax), pch=19, cex = 1, ...)
    
    polygon(c(X_c, rev(X_c)), c(Y_c - std_Y_c, rev(Y_c + std_Y_c)), col = adjustcolor("grey",alpha.f=0.85) , border = 'blue',lwd = 2,  ...)
    polygon(c(X_c, rev(X_c)), c(Y_c - (2*std_Y_c), rev(Y_c + (2*std_Y_c))), col = adjustcolor("grey",alpha.f=0.5) , border = 'red3', lwd = 1, ...)
    points(X_c, Y_c, pch=19, cex = 1) #Redo the points to go on top of the polygons
...
```{r}
#Grab the data first
x = log10(gene_skew_corr_df$avg_expression)
y = gene_skew_corr_df$correlation_to_tissue_skew

#Order on the x axis
ordered_y = y[order(x)]

convolved_gene_skew_exp = conv_smoother(x,y, window = 25, raw = TRUE, output = TRUE, xlab = 'log10(exp)', ylab = 'Gene-tissue skew correlation', main = 'Using EGAD default code')

X_c = convolved_gene_skew_exp[, 'X_c']
Y_c = convolved_gene_skew_exp[, 'Y_c']
std_Y_c = convolved_gene_skew_exp[, 'std_Y_c']

#pdf(file = '/home/werner/xchrom_snp_skew/code/graphs/gene_skew_to_tissue_skew/convolved_gene_expression_to_skew.pdf', useDingbats = FALSE)
#Raw data
plot(x, y, pch = 19, cex = 0.4, col = 'grey30', ylim = c(0,1.1), xlim = c(min(x), max(x)), 
     xlab = 'log10( Average gene expression )', ylab = 'Similarity between gene and tissue skew (corr)', main = 'Gene-tissue skew correlations trend with expression levels')
#Standard deviations and moving average
polygon(c(X_c, rev(X_c)), c(Y_c - std_Y_c, rev(Y_c + std_Y_c)), col = adjustcolor("grey",alpha.f=0.5) , border = 'black',lwd = 2)
polygon(c(X_c, rev(X_c)), c(Y_c - (2*std_Y_c), rev(Y_c + (2*std_Y_c))), col = adjustcolor("grey",alpha.f=0.25) , border = 'black', lwd = 2)
lines(X_c, Y_c, col = 1, lwd = 3)
...
Pull out the genes that do not follow the trend

```{r}
window = 25
convolve_index = floor((window/2):(length(x) - (window/2)))
row_ind = ordered_y[convolve_index] < (Y_c - (2*std_Y_c))
ordered_x = x[order(x)]
bad_x = ordered_x[convolve_index][row_ind]
bad_y = ordered_y[convolve_index][row_ind]


#pdf(file = '/home/werner/xchrom_snp_skew/code/graphs/gene_skew_to_tissue_skew/convolved_gene_expression_to_skew_with_outliers.pdf', useDingbats = FALSE)
plot(x, y, pch = 19, cex = 0.4, col = 'grey30', ylim = c(0,1.1), xlim = c(min(x), max(x)), 
     xlab = 'log10( Average gene expression )', ylab = 'Gene and tissue skew correlation', main = 'Gene-tissue skew correlations trend with expression levels')
#Standard deviations and moving average
polygon(c(X_c, rev(X_c)), c(Y_c - std_Y_c, rev(Y_c + std_Y_c)), col = adjustcolor("grey",alpha.f=0.5) , border = 'black',lwd = 2)
polygon(c(X_c, rev(X_c)), c(Y_c - (2*std_Y_c), rev(Y_c + (2*std_Y_c))), col = adjustcolor("grey",alpha.f=0.25) , border = 'black', lwd = 2)
lines(X_c, Y_c, col = 1, lwd = 3)

#Check i got the right genes
points(bad_x, bad_y, col = 'red3', pch = 19)
#dev.off()

```


Or plot the z_scored values, highlight the outliers

```{r}

zscored_convolved_corrs = (ordered_y[convolve_index] - Y_c) / std_Y_c
hist(zscored_convolved_corrs)

plot(zscored_convolved_corrs,ordered_y[convolve_index] )
...
#ggsave(path = '/home/werner/xchrom_snp_skew/code/graphs/gene_skew_to_tissue_skew/', filename = 'gene_skew_to_tissue_skew_binned_boxplots.pdf', device = 'pdf', 
...
```{r}
index = temp_gene_df$label == 'Binned expression 5'
plot(temp_gene_df$avg_expression[index], temp_gene_df$correlation_to_tissue_skew[index])

```


Grab the outliers from the box plot
```{r}
is_outlier <- function(x) {
  return(x < quantile(x, 0.25) - 1.5 * IQR(x) | x > quantile(x, 0.75) + 1.5 * IQR(x))
}

boxplots = unique(temp_gene_df$label)
all_outliers = c()
for(i in 1:length(boxplots)){
  
  #Get correlations for that boxplot
  corrs = temp_gene_df$correlation_to_tissue_skew[temp_gene_df$label == boxplots[i]]
  outlier_index = is_outlier(corrs)
  outlier_genes = as.character(temp_gene_df$gene_name[temp_gene_df$label == boxplots[i]][outlier_index])
  all_outliers = c(all_outliers, outlier_genes)
...
And the genes that do noot follow the trend are....
Go through the dataset for each gene and get it's skew and the estimated tissue skew withholding it, plot against to see if there's a trend

```{r}
#Outliers based on convolution method
#poor_skew_corr_genes = as.character(gene_skew_corr_df[order(gene_skew_corr_df$avg_expression), 'gene_name'][convolve_index][row_ind])

#Outliers based on boxplot bins
poor_skew_corr_genes = all_outliers
gene_skews = c()
est_tissue_skews = c()
gene_name = c()
sample_index = c()
...
for(j in 1:length(poor_skew_corr_genes)){
  
  current_gene = poor_skew_corr_genes[j]
  
  #Go through the data, and get the index of samples that detect that gene
  sample_index_for_gene= c()
  gene_ref_skew = c()
  for(i in 1:length(list.skew.max)){
    data = list.skew.max[[i]]
    ...
    if(sum(present) == 0) #If detected in that sample, grab the sample index and the gene skew
      {next} else{
        sample_index_for_gene = c(sample_index_for_gene, i)
        #Grab that sample's gene skew for this gene
        gene_ref_skew = c(gene_ref_skew , data[present, 'C.1'])
    }
  }
  ...
  na_index = !is.na(temp_est_skew)
  temp_est_skew = temp_est_skew[na_index]
  gene_ref_skew = gene_ref_skew[na_index]
  sample_index_for_gene = sample_index_for_gene[na_index]
  
  #Add to a total data frame
  gene_skews = c(gene_skews,gene_ref_skew)
  est_tissue_skews = c(est_tissue_skews,temp_est_skew )
  sample_index = c(sample_index, sample_index_for_gene)
  labels = rep(current_gene, length(gene_ref_skew) )
  gene_name = c(gene_name, labels)
  
  
  print(sprintf('Finished %s at %s', 
                current_gene,  Sys.time()))

}

bad_gene_skew_df = data.frame(gene_skews = gene_skews, estimated_tissue_skews = est_tissue_skews, gene_name = gene_name, sample_index = sample_index)
bad_gene_skew_df
```


Plot the gene and tissue skews against each other


```{r}
for( i in 1:length(poor_skew_corr_genes)){
  current_gene = poor_skew_corr_genes[i]
  
  gene_skews = bad_gene_skew_df$gene_skews[bad_gene_skew_df$gene_name == current_gene]
  tissue_skews = bad_gene_skew_df$estimated_tissue_skews[bad_gene_skew_df$gene_name == current_gene]
  title = sprintf('%s tissue vs gene skew', current_gene)
  par(pty = 's')
  plot(tissue_skews, gene_skews, main = title, xlim = c(.5, 1), ylim = c(.5, 1))
  abline(a = 0, b = 1, col = 'red')
}
...
#pdf(file = '/home/werner/xchrom_snp_skew/code/graphs/gene_skew_to_tissue_skew/ranked_gene_skew_to_tissue_skew_correlation.pdf', height = 5, width = 5, useDingbats = FALSE)
...
pdf(file = '/home/werner/xchrom_snp_skew/code/graphs/gene_skew_to_tissue_skew/chrX_map_genes_and_number_of_samps_detected_in_V8.pdf', height = 8, width = 20, useDingbats = FALSE)
...
index = kept_gene_meta$name %in% label_genes
sum(index)
kept_gene_meta$start[index]
```

```{r}
gene_skew_correlations[gene_skew_correlations >= .8]
...
```{r}
#old skews
#oad("/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/bootstrap_results_all_v7_GATK_final_filt_2.Rdata")
...
#load("/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/bootstrap_results_all_v7_GATK_11_5_2020_update.Rdata")

#With escape genes filtered out
#load("/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/bootstrap_results_all_v7_GATK_11_5_2020_update_filt_escape.Rdata")

load("/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/bootstrap_results_all_v8_GATK_1_20_2021_update_filt_escape.Rdata")
```
...
```{r}
library(plotrix)  #For the plotCI() function
```
```{r}
#Looking at the first x samples for all three filters

samples = 250

#Check none of these samples are missing, cause then the samples wouldn't line up when comparing between the different filters
table(is.na(mus[1:samples]))

x = mus[1:samples]
CI_widths_sorted = sort(mu_CI_width[1:samples], decreasing=TRUE)
sorted_y = match(mu_CI_width[1:samples], CI_widths_sorted)


plotCI(x=x, y=sorted_y, uiw=mu_upper_CI[1:samples], liw=mu_lower_CI[1:samples], err='x', xlim=c(.5,1), 
       xlab='MLE mu parameter', ylab='CI rank', main='No filtering',
       sfrac=0)
...
#unlist(mclapply(1:length(subset.list.skew.max), function(i) get_gene_skews(subset.list.skew.max[[i]]), mc.cores = 5))
...
gene_skews = ggplot(unphased_gene_skew_df, aes(x = gene_skews, fill = labels)) + geom_density(alpha = .5)+
  xlab('Gene level skews') +
  #scale_fill_manual(values=c("blue", "gold3")) +
...
ggsave(path = '/home/werner/xchrom_snp_skew/code/graphs/entex_validation_folded_normal/', filename = 'gtex_gene_skews_binned_by_tissue_skews.pdf', device = 'pdf', 
...
#fdr = .01
#BH_value = (skew_and_stats_df$p_val_rank / (dim(skew_and_stats_df)[1])) * fdr     #Get the BH-critical value
#BH_filt = which(skew_and_stats_df$ks_pvals < BH_value )                         #Find the pvals < their BH value
#significant = which.max(skew_and_stats_df$ks_pvals[BH_filt])                    #Grab the max p_val out of those and get its rank
#significant = skew_and_stats_df$p_val_rank[BH_filt[significant]]
#sig_identity = skew_and_stats_df$p_val_rank < significant                       #Get an identity vector for the significant p_values and add it to the data.frame
#skew_and_stats_df$p_significance = sig_identity
...
```{r}
CI_skew_plot = ggplot(skew_and_stats_df, aes(skew, CI_width, color=p_significance)) + geom_point(alpha=1/5,size=1) + ylab('Skew CI width') + xlab('Skew estimate') + ggtitle('Skew estimate vs CI: Final filtering') + theme(plot.title=element_text(hjust=.5)) + labs(color='Bad MLE fit')+ geom_density_2d() + xlim(.5,1) + ylim(0, .45)
ggMarginal(CI_skew_plot, groupColour=TRUE, groupFill=TRUE)



CI_numSNP_plot = ggplot(skew_and_stats_df, aes(num_snps, CI_width, color=p_significance)) + geom_point(alpha=1/5,size=1) + ylab('Skew CI width') + xlab('Number of SNPs used in skew estimate') + ggtitle('Num SNPs vs CI: Final filtering') + theme(plot.title=element_text(hjust=.5)) + labs(color='Bad MLE fit')+ geom_density_2d() + ylim(0, .45) + xlim(0,120)
ggMarginal(CI_numSNP_plot, groupColour=TRUE, groupFill=TRUE)



sigma_numSNP_plot = ggplot(skew_and_stats_df, aes(num_snps, skew_sigma, color=p_significance)) + geom_point(alpha=1/5,size=1) + ylab('Skew sigma') + xlab('Number of SNPs used in skew estimate') + ggtitle('Num SNPs vs sigma: Final filtering') + theme(plot.title=element_text(hjust=.5)) + labs(color='Bad MLE fit')+ geom_density_2d() + ylim(0, .45) + xlim(0,120)
ggMarginal(sigma_numSNP_plot, groupColour=TRUE, groupFill=TRUE)
...
Looking at the cumulative ref ratio density plots for good and bad fits, high and low confidence


```{r}

f_bad_low = (skew_and_stats_df$p_significance == TRUE) & (skew_and_stats_df$CI_width >= .05 )
f_bad_high = (skew_and_stats_df$p_significance == TRUE) & (skew_and_stats_df$CI_width < .05 )
f_good_low = (skew_and_stats_df$p_significance == FALSE) & (skew_and_stats_df$CI_width >= .05 )
f_good_high = (skew_and_stats_df$p_significance == FALSE) & (skew_and_stats_df$CI_width < .05)

l = vector(mode='character', length=length(f_bad_low))

l[f_bad_low] = 'Bad fit Low confidence'
l[f_bad_high] = 'Bad fit High confidence'
l[f_good_low] = 'Good fit Low confidence'
l[f_good_high] = 'Good fit High confidence'

skew_and_stats_df$sum_label = l


#Pooling together all the SNP skews from all samples in the different Bad/Good fit High/Low confidence categories


index = skew_and_stats_df[f_bad_low, 'sample_index']
bad_low_num = length(index)
high.f = which(list.skew.max[[index[1]]]$A.1 < 3000)
bad_low = list.skew.max[[index[1]]]$C.1[high.f]
for(i in 2:length(index)){
  high.f = which(list.skew.max[[index[i]]]$A.1 < 3000)
  bad_low = c(bad_low, list.skew.max[[index[i]]]$C.1[high.f])
}


index = skew_and_stats_df[f_bad_high, 'sample_index']
bad_high_num = length(index)
high.f = which(list.skew.max[[index[1]]]$A.1 < 3000)
bad_high = list.skew.max[[index[1]]]$C.1[high.f]
for(i in 2:length(index)){
  high.f = which(list.skew.max[[index[i]]]$A.1 < 3000)
  bad_high = c(bad_high, list.skew.max[[index[i]]]$C.1[high.f])
}

index = skew_and_stats_df[f_good_low, 'sample_index']
good_low_num = length(index)
high.f = which(list.skew.max[[index[1]]]$A.1 < 3000)
good_low = list.skew.max[[index[1]]]$C.1[high.f]
for(i in 2:length(index)){
  high.f = which(list.skew.max[[index[i]]]$A.1 < 3000)
  good_low = c(good_low, list.skew.max[[index[i]]]$C.1[high.f])
}

index = skew_and_stats_df[f_good_high, 'sample_index']
good_high_num = length(index)
high.f = which(list.skew.max[[index[1]]]$A.1 < 3000)
good_high = list.skew.max[[index[1]]]$C.1[high.f]
for(i in 2:length(index)){
  high.f = which(list.skew.max[[index[i]]]$A.1 < 3000)
  good_high = c(good_high, list.skew.max[[index[i]]]$C.1[high.f])
}


a = rep('Bad fit Low confidence',length(bad_low))
b = rep('Bad fit High confidence',length(bad_high))
c = rep('Good fit Low confidence',length(good_low))
d = rep('Good fit High confidence',length(good_high))


summary_skews = c(bad_low, bad_high, good_low, good_high)
summary_label = c(a,b,c,d)
summary_density_df = data.frame(summary_skews = summary_skews, summary_label = summary_label)


ggplot(summary_density_df[summary_density_df$summary_label %in% c('Good fit High confidence','Good fit Low confidence'), ], aes(x=summary_skews, fill=summary_label)) + geom_density(alpha=.5) + ggtitle('Final filtering: Good folded_norm fits') + xlab('Ref / Total') + xlim(0,1) + theme(plot.title=element_text(hjust=.5))


ggplot(summary_density_df[summary_density_df$summary_label %in% c('Bad fit High confidence','Bad fit Low confidence'), ], aes(x=summary_skews, fill=summary_label)) + geom_density(alpha=.5) + ggtitle('Final filtering: Bad folded_norm fits') + xlab('Ref / Total') + xlim(0,1) + theme(plot.title=element_text(hjust=.5))

```

```{r}
sum(f_bad_low[!is.na(f_bad_low)])  
sum(f_bad_high[!is.na(f_bad_high)])
sum(f_good_low[!is.na(f_good_low)]) 
sum(f_good_high[!is.na(f_good_high)]) 
...
And what if I use TSIX or XIST skews as the skew estimate for the tissue for the samples that fail the KS-test for the folded normal distribution?
...
bad_fold_norm_samples = skew_and_stats_df$sample_index[skew_and_stats_df$p_significance == TRUE]
par(pty = 's')
hist(est_skew, breaks = 32, xlim = c(.5,1), main = 'Original tissue skews')
...
#Replace the skews of samples that are bad folded normal fits and also express XIST with the XIST skew
temp_skews_xist = est_skew
bad_fits_and_xist = sample_index_for_gene[sample_index_for_gene %in% bad_fold_norm_samples] #The samples that are bad and also express XIST
temp_skews_xist[bad_fits_and_xist] = folded(re_gene_ref_skew[bad_fits_and_xist]) #Replace with XIST skews

hist(temp_skews_xist, breaks = 32, xlim = c(.5,1), main = 'Replacing bad fits with XIST skews')
plot(est_skew, temp_skews_xist, xlim = c(.5,1), main = 'Original skews vs replaced XIST skews', xlab = 'Original skews', ylab = 'XIST skews')
abline(a=0,b=1,col = 'red')
...
temp_skews_tsix = est_skew
bad_fits_and_tsix = sample_index_for_gene[sample_index_for_gene %in% bad_fold_norm_samples] 
temp_skews_tsix[bad_fits_and_tsix] = folded(re_gene_ref_skew[bad_fits_and_tsix])

hist(temp_skews_tsix, breaks = 32, xlim = c(.5,1), main = 'Replacing bad fits with TSIX skews')
plot(est_skew, temp_skews_tsix, xlim = c(.5,1), main = 'Original skews vs replaced TSIX skews', xlab = 'Original skews', ylab = 'TSIX skews')
abline(a=0,b=1,col = 'red')
...
#save(skew_and_stats_df, file = "/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/final_filtering_skew_and_stats_df.Rdata")

#save(skew_and_stats_df, file = "/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/final_filtering_skew_and_stats_11_5_20_df.Rdata")

#save(skew_and_stats_df, file = "/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/final_filtering_skew_and_stats_11_5_20_escape_filt_df.Rdata")
...
Compare the skew estimates using the correct data (fixed annotations and SNP filtering) to the original skew estimates
```{r}
#load("/home/werner/xchrom_snp_skew/data/GTEx/snp_skews/GATK_mod/final_filtering_skew_and_stats_df.Rdata")
...
```{r}
#Samples that had skews in the original SNP list, which was wrong
index = skew_and_stats_df$sample_index
#Updated presumably corect skews
updated_skews = est_skew[index]

old_skews =skew_and_stats_df$skew

par(pty = 's')
plot(old_skews, updated_skews, cex=.25, xlim = c(.5,1), ylim = c(.5,1))
...
t = 1:length(est_skew)
new_samples = t[!t%in%index]  #Getting the samples that were previously filtered out in the original SNP list
hist(est_skew[new_samples], main = 'Skews of samples previously filtered out', xlab = 'Skew estimates')
